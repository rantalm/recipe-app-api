...
class PrivateTagsApiTests(TestCase):
    ...
    def test_user_cannot_delete_another_user_tag(self):
        """Test that a user cannot delete another user tag."""
        user2 = create_user('user2@examplke.com')
        tag = Tag.objects.create(user=user2, name='Breakfast')

        url = detail_url(tag.id)
        res = self.client.delete(url)

        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
        tags = Tag.objects.filter(user=user2)
        self.assertTrue(tags.exists())


    def test_filter_tags_assigned_to_recipes(self):
        """Test listing tags by thos assign to recipes."""
        in1 = Tag.objects.create(user=self.user, name='Apples')
        in2 = Tag.objects.create(user=self.user, name='Turkey')
